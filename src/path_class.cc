...
void Path::Init(Input &in, IO &out, RNG &rng)
{
  ...
  n_d = in.GetChild("System").GetAttribute<uint>("n_d");
  out.Write("System/n_d",n_d);
  n_bead = in.GetChild("System").GetAttribute<uint>("n_bead");
  ...
  n_part = 0;
  for (uint s_i=0; s_i<n_species; s_i++) {
...
void Path::PrintPath()
{
  SetMode(0);
  for (uint s_i=0; s_i<n_species; ++s_i) {
    for (uint p_i=0; p_i<species_list[s_i]->n_part; ++p_i) {
      for (uint b_i=0; b_i<n_bead; ++b_i) {
        std::cout << p_i << " " << b_i << " ";
...
// Get species info
void Path::GetSpeciesInfo(const std::string &species, uint &species_i)
{
  for (uint s_i=0; s_i<n_species; s_i++) {
...
void Path::SetupKs(const double k_cut)
{
  ...
  for (uint d_i=0; d_i<n_d; d_i++)
    max_k_index(d_i) = (uint) ceil(1.1*k_cut/k_box(d_i));
...
void Path::UpdateRhoKP(const uint b0, const uint b1, std::vector<std::pair<uint,uint>> const& particles, const uint level)
{
  uint skip = 1<<level;

  // Get species numbers
  std::vector<uint> species, ps;
...
// Update rho k values for specific particles and slices
void Path::UpdateRhoKP(const uint b0, const uint b1, uint s_i, std::vector<uint> const& particles, const uint level)
{
  bool tmpMode = GetMode();
  SetMode(1);
  uint skip = 1<<level;
...
// Update rho k values for specific particles and slices
void Path::UpdateRhoK(const uint b0, const uint b1, std::vector<std::pair<uint,uint>> const& particles, const uint level)
{
  bool tmpMode = GetMode();
  uint skip = 1<<level;

  // Get species numbers
  std::vector<uint> species;
  for (auto& p: particles) {
    uint s_i = p.first;
    uint p_i = p.second;
    species.push_back(s_i);
...
void Path::CalcC(vec<double> const& r)
{
  for (uint d_i=0; d_i<n_d; d_i++) {
    ...
    for (uint k_i=1; k_i<=max_k_index(d_i); k_i++) {
...
void Path::AddRhoKP(field<vec<std::complex<double>>> &tmp_rho_k, const uint p_i, const uint b_i, const uint s_i, const int pm)
{
  vec<double> r = GetR((*this)(s_i,p_i,b_i));
  CalcC(r);
  for (uint k_i=0; k_i<k_indices.size(); k_i++) {
...
void Path::SetCycleCount(const uint s_i, std::vector<uint> &cycles)
{
  vec<uint> already_counted;
...
uint Path::GetPermSector(const uint s_i)
{
  std::vector<uint> cycles;
  SetCycleCount(s_i, cycles);
...
// Retrieve permutation sector
uint Path::GetPermSector(const uint s_i, std::vector<uint> &cycles)
{
...
void Path::SetupPermSectors(const uint n, const uint sectors_max)
{
  if (!perm_sectors_setup) {
    ...
    int k = 1;
    int y = n-1;
    std::vector<std::vector<uint>> tmp_poss_perms;

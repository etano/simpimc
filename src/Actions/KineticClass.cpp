...
void Kinetic::SetupSpline()
{
  ...
  for (uint iSpline=0; iSpline<nSpline; ++iSpline) {
    ...
    for (uint i=0; i<r_grid.num; ++i) {
      ...
      if (iSpline == 0)
        num_sum_r(i) = 0.;
      for (uint image=-nImages; image<=nImages; image++) {
        if (image != 0) {
...
double Kinetic::GetGaussSum(const double r, const double r2i4LambdaTau, const uint sliceDiff)
{
  double gaussSum;
  eval_UBspline_1d_d(rho_free_r_splines(sliceDiff-1),r,&gaussSum);
  return exp(-(r2i4LambdaTau/sliceDiff))*exp(0.9999*gaussSum);
}

double Kinetic::GetNumSum(const double r, const double r2i4LambdaTau)
{
  double numSum;
  eval_UBspline_1d_d(num_sum_r_spline,r,&numSum);
  return -(r2i4LambdaTau/path.tau)*exp(-r2i4LambdaTau)*exp(0.9999*numSum);
}
  ...
  for (uint iP=0; iP<nPart; iP++) {
    for (uint iB=0; iB<path.nBead; iB++) {
      ...
      for (uint iD=0; iD<path.nD; iD++) {
        double r2i4LambdaTau = dr(iD)*dr(iD)*i4LambdaTau;
        numSum(iD) = GetNumSum(dr(iD),r2i4LambdaTau);
        gaussSum(iD) = GetGaussSum(dr(iD),r2i4LambdaTau,1);
        gaussProd *= gaussSum(iD);
      }
      double scalarNumSum = 0.;
      for (uint iD=0; iD<path.nD; iD++) {
        double numProd = 1.;
        for (uint jD=0; jD<path.nD; jD++) {
          if (iD != jD)
            numProd *= gaussSum(jD);
          else
            numProd *= numSum(jD);
...
double Kinetic::GetAction(const uint b0, const uint b1, const vector<pair<uint,uint> >& particles, const uint level)
{
  uint skip = 1<<level;
  ...
  for (auto& p: particles) {
    ...
    uint iP = p.second;
    if (iS == iSpecies) {
      std::shared_ptr<Bead> beadA(path(iS,iP,b0));
      ...
      while(beadA != beadF) {
        ...
        for (uint iD=0; iD<path.nD; iD++) {
          double r2i4LambdaTau = dr(iD)*dr(iD)*i4LambdaTau;
          gaussProd *= GetGaussSum(dr(iD),r2i4LambdaTau,skip);
        }
        tot -= log(gaussProd);
        beadA = beadB;
      }
...
vec<double> Kinetic::GetActionGradient(const uint b0, const uint b1, const vector<pair<uint,uint> >& particles, const uint level)
{
  uint skip = 1<<level;
  double i4LambdaLevelTau = i4LambdaTau/skip;
  vec<double> tot;
...
double Kinetic::GetActionLaplacian(const uint b0, const uint b1, const vector<pair<uint,uint> >& particles, const uint level)
{
  uint skip = 1<<level;
  double i4LambdaLevelTau = i4LambdaTau/skip;
  double tot = 0.;

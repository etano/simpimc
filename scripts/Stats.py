...
def C(g,t,mean,var,N):
  if (var == 0):
    return 0
  diff = [x-mean for x in g]
  total = sum([x*y for (x,y) in zip(diff[0:N-t],diff[t:N])])
  return (1/(var*(N-t)))*total
...
# Squared Mean
def Mean2(g):
  return sum([x**2 for x in g])/len(g)
...
# Mean Squared Root
def MSR(g):
  return sqrt(sum([x**2 for x in g]))/len(g)
...
# Error with auto-correlation correction
def Error(g):
...
# Average means with error weights
...
def StdStats(g):
  ...
  try:
    ...
    var = 0.
    stddev = 0.
  return [mean,stddev,stddev/sqrt(N)]
...
# Block analysis to propagate errors
def BlockAnalysis(g):
  N = len(g)
